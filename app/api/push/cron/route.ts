...
export async function GET(request: Request) {
  ...
  for (const profile of rows ?? []) {
    ...
    if (
      ...
      !hasActiveShift && // only if NOT clocked in
      profile.clock_in_notified_date !== dateStr && // not already sent today
    ...
    if (
      ...
      !hasActiveShift && // only if NOT currently clocked in (already stopped or never started)
